...
export class TestsService {
  ...
  questions(slug: string) {
    return this.prisma.testDefinition.findUnique({
      where: { slug },
      include: {
        questions: {
  ...
  async submit(dto: SubmitTestDto) {
    const definition = await this.prisma.testDefinition.findUnique({
      where: { id: dto.testDefinitionId, isActive: true },
      include: { questions: true },
    });
    if (!definition) throw new NotFoundException("Test not found");
    if (
      dto.answers.length !== definition.questions.length ||
      new Set(dto.answers.map((a) => a.questionId)).size !==
    ...
    for (const q of definition.questions) {
      ...
      if (
        ...
        throw new BadRequestException(`Invalid answer for ${q.code}`);
      const value = q.reverseScored
        ? (q.maxValue ?? 5) + (q.minValue ?? 1) - raw
        : raw;
      const entry = sums.get(q.trait) ?? { total: 0, weight: 0 };
      entry.total += value * q.weight;
      entry.weight += q.weight;
      sums.set(q.trait, entry);
    }
        ...
        data: {
          ...
          responses: {
            create: dto.answers.map((a) => ({
              ...
              value: a.value as unknown as Prisma.InputJsonValue,
            })),
          },
          traitScores: {
            create: [...sums.entries()].map(([trait, value]) => ({
              trait,
              score: (value.total / value.weight - 1) / 4,
            })),
          },
        },
